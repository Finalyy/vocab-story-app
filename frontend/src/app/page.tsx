    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 no-print" onClick={onClose}>
      <div className="bg-[#2B3245] rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in-95 flex flex-col md:flex-row text-slate-100 border border-slate-600" onClick={e => e.stopPropagation()}>
        <div className="md:w-1/3 bg-[#242A38] p-6 md:p-8 flex flex-col items-start border-b md:border-b-0 md:border-r border-slate-700 relative">
          ...
          {loading && <div className="mt-8 flex items-center gap-2 text-indigo-400 text-sm bg-indigo-900/20 px-3 py-2 rounded-lg"><Loader2 size={16} className="animate-spin" /> Đang trích xuất...</div>}
        </div>
        <div className="md:w-2/3 p-6 md:p-8 space-y-4 max-h-[70vh] overflow-y-auto text-[15px] leading-relaxed relative bg-[#2B3245] custom-scrollbar-dark">
          ...
          {!loading && (
            <div className="space-y-4 pt-2">
              <DictRow label="VN" value={wordData.meaning} /><DictRow label="EN" value={wordData.en_meaning} /><DictRow label="Ví dụ" value={wordData.example} /><DictRow label="Họ từ" value={wordData.word_family} /><DictRow label="Đồng nghĩa" value={wordData.synonyms} /><DictRow label="Trái nghĩa" value={wordData.antonyms} /><DictRow label="Cụm thường gặp" value={wordData.collocations} /><DictRow label="Dễ nhầm lẫn" value={wordData.confusions} />
              {!wordData.meaning && !wordData.synonyms && !wordData.word_family && (
                <div className="text-slate-400 italic flex items-center gap-2"><Info size={16} /> Tài liệu gốc không cung cấp dữ liệu cho từ này.</div>
              )}
            </div>
          )}
        </div>
            ...
            <div className="md:col-span-5 space-y-6 no-print">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <FileUploader 
                  ...
                  setExternalLoading={setFileLoading} 
                  setExternalStatus={setUploadStatus} 
                />
                ...
                <textarea className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-slate-700 mt-2" rows={4} placeholder="Sửa/thêm từ vựng..." value={inputText} onChange={(e) => setInputText(e.target.value)}></textarea>
